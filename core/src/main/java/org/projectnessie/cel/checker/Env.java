...
public class Env {
  ...
  private Env(Container container, TypeProvider provider, Scopes declarations, int aggLitElemType) {
  ...
  public static Env newEnv(Container container, TypeProvider provider) {
    Scopes declarations = newScopes();
    declarations.push(); // TODO why this ??

    return new Env(container, provider, declarations, dynElementType);
  }

  /** NewStandardEnv returns a new *Env with the given params plus standard declarations. */
  public static Env newStandardEnv(Container container, TypeProvider provider) {
    Env e = newEnv(container, provider);
    e.add(Standard.StandardDeclarations);
   ...
   */
  public Env enableDynamicAggregateLiterals(boolean enabled) {
  ...
  Env enterScope() {
    ...
    return new Env(this.container, this.provider, childDecls, this.aggLitElemType);
  }

  // exitScope creates a new Env instance with the nearest outer declaration scope.
  Env exitScope() {
    Scopes parentDecls = declarations.pop();
    return new Env(this.container, this.provider, parentDecls, this.aggLitElemType);
  }
